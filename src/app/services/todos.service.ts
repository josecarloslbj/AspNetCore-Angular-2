...
export class TodoService {
    private baseUrl: string = "http://localhost:49414/api/todo";
    ...
    public post(todo: Todo): Observable<GenericResult<Todo>> {
        ...
        return this.http.post(this.baseUrl, JSON.stringify(todo), { headers })
            .map(res => res.json());
    }
    ...
    public put(id: number, todo: Todo): Observable<GenericSimpleResult> {
        let headers = new Headers();
        headers.append('Content-Type', 'application/json');
        return this.http.put(this.baseUrl + '/' + id, JSON.stringify(todo), { headers })
            .map(res => res.json());
    }
